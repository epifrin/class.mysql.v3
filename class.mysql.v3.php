<?php
/**  
 *     ласс MySQLi дл€ работы с базой данных MySQL
 *    ƒл€ PHP5
 *    @version: 3.1.3
 *    @update: 01.08.2012
...
class MySQLQuery3{
    ...
    function __construct($dblocation, $dbname, $dbuser, $dbpass, $charset = ''){
        ...
        if ($dbcon){
            ...
            echo "<p>Ѕаза данных недоступна, работа сайта невозможна!!!</p>";
            exit();
        }
    }
    // закрыть соединение
    ...
    // проверка значени€ дл€ пол€ date в mysql
    ...
    // проверка значени€ дл€ пол€ time в mysql
    ...
    // проверка значени€ дл€ пол€ datetime в mysql
                    ...
                    if(!empty($arr_params[$field])){
                        ...
                        if(($arr_params[$field] == 'NULL') && !empty($struct['empty'])){
                            ...
                            if(is_numeric($arr_params[$field]) && (int)$arr_params[$field]){
                                ...
                                $this->err_text = 'Ќедопустимое значение в параметре "'.$field.'"<br>';
                            }
                        }elseif($struct['type'] == 'text'){
                        ...
                        $this->err_text = 'Ќе задано значенние параметра "'.$field.'"<br>';
                    }
                
                }elseif(empty($struct['empty']) && empty($struct['pk'])){
                    if($struct['if_no_param'] == 'insert_default' || $struct['if_no_param'] == 'default'){
                        if(!isset($struct['default']) && $struct['type'] != 'date' && $struct['type'] != 'datetime' && $struct['type'] != 'time' ) $this->err_text = 'ƒл€ параметра "'.$field.'" не задано значение по-умолчанию<br>'; 
                    }else{
                        $this->err_text = 'Ќет параметра "'.$field.'"<br>';
                    }
                }
            }
            if($this->err_text){ 
    ...
    * @param string $table
    * @param string $where_field  // поле, дл€ услови€ WHERE 
    * @return bool
    */
    ...
    // получить структуру таблицы дл€ php-кода
    ...
    private function db_error($msg,$text_sql, $arr_debug){
        ...
        $err_msg = '«апрос: '.$text_sql.'<br> ќшибка: '.$msg.' <br>';
        $err_msg .= '÷епочка вызываемых скриптов:<br>';
        foreach($arr_debug AS $debug){
            $err_msg .= '—крипт: '.$debug['file'].', —трока: '.$debug['line'].'<br>';
        }
        echo str_replace("\n",'<br>',$err_msg);
        /*$query = mysqli_query("SELECT value FROM settings WHERE var='admin_mail'");
        $mail_admin = @mysqli_result($query, 0, 'value');
        if (!empty($mail_admin)){
            if (!mail($mail_admin, "site: ќшибка MySQL", $err_msg, "Content-Type: text/plain; charset=windows-1251\r\nFrom: mysql@domain.com\r\n" . "Reply-To: mysql@domain.com\r\n" . "X-Mailer: PHP/" . phpversion()))
            {
                echo $err_msg;
            }
        }//*/
        exit("¬озникла ошибка при работе с базой данных! ќбратитесь к администратору сайта.");
    }
}
